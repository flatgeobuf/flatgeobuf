...
public class FeatureCollectionConversionsTest {
    ...
    public void countriesTestFilter() throws IOException {
        File file = new File("../../test/data/countries.fgb");
        FileChannel fileChannel = (FileChannel) Files.newByteChannel(file.toPath(), EnumSet.of(StandardOpenOption.READ));
        MappedByteBuffer mappedByteBuffer = fileChannel.map(FileChannel.MapMode.READ_ONLY, 0, fileChannel.size());
        mappedByteBuffer.order(ByteOrder.LITTLE_ENDIAN);
        Envelope rect = new Envelope(12, 12, 56, 56);
        Iterator<SimpleFeature> it = FeatureCollectionConversions.deserialize(mappedByteBuffer, rect).iterator();
        int count = 0;
        while (it.hasNext()) {
            it.next();
            count++;
