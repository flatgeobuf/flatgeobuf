...
TEST_CASE("PackedRTree")
{
    SECTION("PackedRTree 2 items 2")
    {
        ...
        REQUIRE(list.size() == 1);
        REQUIRE(nodes[list[0].offset/sizeof(NodeItem)].intersects({0, 0, 1, 1}) == true);
    }
    ...
    SECTION("PackedRTree 2 rectitems 2")
    {
        ...
        auto list = tree.search(0, 0, 1, 1);
        REQUIRE(list.size() == 1);
        REQUIRE(items[list[0].offset/sizeof(NodeItem)]->nodeItem.intersects({0, 0, 1, 1}) == true);
    }
    ...
    SECTION("PackedRTree 19 items + roundtrip + streamSearch")
    {
        ...
        for (uint32_t i = 0; i < list.size(); i++) {
            REQUIRE(nodes[list[i].offset/sizeof(NodeItem)].intersects({102, 102, 103, 103}) == true);
        }
        ...
        for (uint32_t i = 0; i < list2.size(); i++) {
            REQUIRE(nodes[list2[i].offset/sizeof(NodeItem)].intersects({102, 102, 103, 103}) == true);
        }
        ...
        for (uint32_t i = 0; i < list3.size(); i++) {
            REQUIRE(nodes[list3[i].offset/sizeof(NodeItem)].intersects({102, 102, 103, 103}) == true);
        }
    }
    ...
    SECTION("PackedRTree 100 000 items in denmark")
    {
        ...
        for (uint64_t i = 0; i < list.size(); i++)
            CHECK(nodes[list[i].offset/sizeof(NodeItem)].intersects({690407, 6063692, 811682, 6176467}) == true);
        ...
        for (uint64_t i = 0; i < list2.size(); i++)
            CHECK(nodes[list2[i].offset/sizeof(NodeItem)].intersects({690407, 6063692, 811682, 6176467}) == true);
    }
}