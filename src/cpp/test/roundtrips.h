...
TEST_CASE("Geometry roundtrips")
{
    ...
    SECTION("Points spatial query")
    {
        ...
        const auto fc1 = deserialize(flatgeobuf.data(), Node { 0, 0, 1000, 1000 });
        REQUIRE(4 == fc1.size());
        const auto fc2 = deserialize(flatgeobuf.data(), Node { 0, 0, 1, 1 });
        REQUIRE(1 == fc2.size());
        const auto fc3 = deserialize(flatgeobuf.data(), Node { 10, 10, 100, 100 });
        REQUIRE(2 == fc3.size());
    }
    ...
    SECTION("poly_landmarks spatial query")
    {
        ...
        std::vector<uint8_t> flatgeobuf;
        serialize(flatgeobuf, expected, true);
        const auto fc1 = deserialize(flatgeobuf.data(), Node { 0, 0, 1, 1 });
        REQUIRE(0 == fc1.size());
        const auto fc2 = deserialize(flatgeobuf.data(), Node { -73.996035, 40.730647, -73.987054,40.738246 });
        REQUIRE(4 == fc2.size());
    }
}
