...
use geozero::{ColumnValue, GeozeroDatasource};
...
fn json_to_fgb() -> std::io::Result<()> {
    ...
    let mut fgb = FgbWriter::new(&mut file, "countries", GeometryType::MultiPolygon);
    fgb.write_magic()?;
    fgb.add_column("fid", ColumnType::ULong);
    ...
    fgb.header_args.features_count += 1; //FIXME
    fgb.write_header()?;
    fgb.write_feature()?;
    ...
    fgb.add_feature_geom(geom).ok();
    fgb.set_property(0, "fid", &ColumnValue::Long(43)).ok();
    fgb.set_property(1, "name", &ColumnValue::String("South Africa"))
        .ok();
    fgb.write_feature()?;
